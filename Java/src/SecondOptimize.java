...
public class SecondOptimize {
	...
	public static void allPentominoes() throws FileNotFoundException {
		...
		ArrayList<ArrayList<Long>> allTiles = new ArrayList<ArrayList<Long>>();
		allTiles.add(O);
		allTiles.add(P);
		allTiles.add(Q);
		allTiles.add(R);
		...
		all(allTiles, tiles, words, 0);
		
	}
	
	public static void all(ArrayList<ArrayList<Long>> allTiles, ArrayList<Long> tiles, ArrayList<Long> words, int level) {
		if(level == 12) {
			tile(tiles, words, "");
		}else {
			ArrayList<Long> current = allTiles.get(level);
			for(long l : current) {
				tiles.add(l);
				all(allTiles, tiles, words, level+1);
