...
public class ThirdOptimize {
	...
	public static void main(String[] args) throws Throwable {
		...
		System.err.println("a(nother) tiling of the poem could not be found");
	...
	public static void select(ArrayList<ArrayList<Long>> wordSets, int level, long board, String sol) {
		if (level == 12) {
			...
			for (long tile : tiles) {
				if ((tile & board) == 0l) {
					select(wordSets, level + 1, tile | board, sol + " " + Long.toString(tile,16));
				}
			}
		}
	}
